"""
Factor Pipeline using Polars for efficient factor computation.
...
class FactorPipeline:
    """
    Efficient factor computation pipeline using Polars.
    Implements vectorized operations for fast factor calculations.
    """
    
    def __init__(self):
        """Initialize the factor pipeline."""
    ...
    def compute_market_factors_mlq(self, raw_data: pl.DataFrame, 
        ...
        """
        # Start with the ticker and date columns
        factors = raw_data.select(['Symbol', 'Dates'])
    ...
    def compute_fundamental_factors(self, raw_data: pl.DataFrame) -> pl.DataFrame:
        ...
        """
        # Start with the Symbol and Dates columns
        factors = raw_data.select(['Symbol', 'Dates'])
        
        # Book-to-Price ratio (BP) - using bpsF_spot and p_price_spot
        if (self.factor_dict['bpsF_spot'] in raw_data.columns and 
            self.factor_dict['p_price_spot'] in raw_data.columns):
            factors = factors.with_columns(
                (pl.col(self.factor_dict['bpsF_spot']).cast(pl.Float64) / 
                 pl.col(self.factor_dict['p_price_spot']) / 1000).alias('BP')
            )
            # Apply filtering conditions
            factors = factors.with_columns(
                pl.when((pl.col('BP') != 0) & (pl.col('BP') >= -10) & (pl.col('BP') <= 10))
                .then(pl.col('BP'))
                .otherwise(pl.lit(None))
                .alias('BP')
            )
        
        # Asset-to-Capital ratio (AssetP) - using totalEQ_0 and tmv
        if (self.factor_dict['totalEQ_0'] in raw_data.columns and 
            self.factor_dict['tmv'] in raw_data.columns):
            factors = factors.with_columns(
                (pl.col(self.factor_dict['totalEQ_0']) / 
                 pl.col(self.factor_dict['tmv']) / 1000).alias('AssetP')
            )
            # Apply filtering conditions (simplified - would need sector info)
            factors = factors.with_columns(
                pl.when((pl.col('AssetP') >= -10) & (pl.col('AssetP') <= 10))
                .then(pl.col('AssetP'))
                .otherwise(pl.lit(None))
                .alias('AssetP')
            )
        
        # Sales-to-Capitalization ratio (SalesP) - using salseQ_0 and tmv
        if (self.factor_dict['salseQ_0'] in raw_data.columns and 
            self.factor_dict['tmv'] in raw_data.columns):
            factors = factors.with_columns(
                (pl.col(self.factor_dict['salseQ_0']) / 
                 pl.col(self.factor_dict['tmv']) / 1000).alias('SalesP')
            )
            # Apply filtering conditions
            factors = factors.with_columns(
                pl.when((pl.col('SalesP') >= -10) & (pl.col('SalesP') <= 10))
                .then(pl.col('SalesP'))
                .otherwise(pl.lit(None))
                .alias('SalesP')
            )
        
        # Return on Equity (ROE) - using earningsQ_0 and totalEQ_0
        if (self.factor_dict['earningsQ_0'] in raw_data.columns and 
            self.factor_dict['totalEQ_0'] in raw_data.columns):
            factors = factors.with_columns(
                (pl.col(self.factor_dict['earningsQ_0']) / 
                 pl.col(self.factor_dict['totalEQ_0'])).alias('ROE')
            )
            # Apply filtering conditions
            factors = factors.with_columns(
                pl.when((pl.col('ROE') >= -10) & (pl.col('ROE') <= 10))
                .then(pl.col('ROE'))
                .otherwise(pl.lit(None))
                .alias('ROE')
            )
        
        # EBITDA yield (EBITDAY) - using cashFlowQ_0 as proxy for EBITDA
        if (self.factor_dict['cashFlowQ_0'] in raw_data.columns and 
            self.factor_dict['tmv'] in raw_data.columns):
            factors = factors.with_columns(
                (pl.col(self.factor_dict['cashFlowQ_0']) / 1000 / 
                 pl.col(self.factor_dict['tmv'])).alias('EBITDAY')
            )
            # Apply filtering conditions
            factors = factors.with_columns(
                pl.when((pl.col('EBITDAY') >= -10) & (pl.col('EBITDAY') <= 10))
                .then(pl.col('EBITDAY'))
                .otherwise(pl.lit(None))
                .alias('EBITDAY')
            )
        
        # Earnings Yield 12-month (EY_12M) - using earningsQ_0 and tmv
        if (self.factor_dict['earningsQ_0'] in raw_data.columns and 
            self.factor_dict['tmv'] in raw_data.columns):
            factors = factors.with_columns(
                (pl.col(self.factor_dict['earningsQ_0']) / 
                 pl.col(self.factor_dict['tmv']) * 100).alias('EY_12M')
            )
            # Apply filtering conditions
            factors = factors.with_columns(
                pl.when((pl.col('EY_12M') <= 10) & (pl.col('EY_12M') >= -10))
                .then(pl.col('EY_12M'))
                .otherwise(pl.lit(None))
                .alias('EY_12M')
            )
        
        # Dividend Yield (DY) - using div_0 and tmv
        if (self.factor_dict['div_0'] in raw_data.columns and 
            self.factor_dict['tmv'] in raw_data.columns):
            factors = factors.with_columns(
                (pl.col(self.factor_dict['div_0']) / 
                 pl.col(self.factor_dict['tmv']) / 1000).alias('DY')
            )
        
        # Financial Leverage - using totalEQ_0 and dbtRtQ_ as proxy
        if (self.factor_dict['totalEQ_0'] in raw_data.columns and 
            self.factor_dict['dbtRtQ_'] in raw_data.columns):
            factors = factors.with_columns(
                (pl.col(self.factor_dict['totalEQ_0']) / 
                 pl.col(self.factor_dict['dbtRtQ_'])).alias('Leverage')
            )
            # Apply filtering conditions
            factors = factors.with_columns(
                pl.when((pl.col('Leverage') <= 20) & (pl.col('Leverage') >= -20))
                .then(pl.col('Leverage'))
                .otherwise(pl.lit(None))
                .alias('Leverage')
            )
        
        # Operating Profit Margin (OPM) - using earningsQ_0 and salseQ_0
        if (self.factor_dict['earningsQ_0'] in raw_data.columns and 
            self.factor_dict['salseQ_0'] in raw_data.columns):
            factors = factors.with_columns(
                (pl.col(self.factor_dict['earningsQ_0']) / 
                 pl.col(self.factor_dict['salseQ_0'])).alias('OPM')
            )
        
        # Value factors - Book to Price (f_value)
        if (self.factor_dict['bpsF_spot'] in raw_data.columns and 
            self.factor_dict['p_price_spot'] in raw_data.columns):
            factors = factors.with_columns(
                (pl.col(self.factor_dict['bpsF_spot']).cast(pl.Float64) / 
        ...
        # Volatility factors (f_vol) - using available volatility measures
        if (self.factor_dict['retStdev_20_spot'] in raw_data.columns and 
            self.factor_dict['retStdev_120_spot'] in raw_data.columns):
            factors = factors.with_columns(
                ((pl.col(self.factor_dict['retStdev_20_spot']) + 
    ...
    def compute_all_factors(self, raw_data: pl.DataFrame, past1m_data: pl.DataFrame = None,
                            past3m_data: pl.DataFrame = None, past12m_data: pl.DataFrame = None) -> pl.DataFrame:
        """
        Compute all factors.
        
        Args:
            raw_data: DataFrame with all required raw data.
            past1m_data: 1-month ago data (for MLQ factors).
            past3m_data: 3-month ago data (for MLQ factors).
            past12m_data: 12-month ago data (for MLQ factors).
            
        Returns:
            DataFrame with all computed factors.
        """
        # Start with the original data
        all_factors = raw_data.select(pl.all())
        
        # Add fundamental factors
        fundamental_factors = self.compute_fundamental_factors(raw_data)
        # Get only the computed factor columns (exclude original data columns)
        fundamental_cols = [col for col in fundamental_factors.columns 
                           if col not in ['Symbol', 'Dates'] and col in fundamental_factors.columns]
        if fundamental_cols:
            for col in fundamental_cols:
                if col in fundamental_factors.columns:
                    all_factors = all_factors.with_columns(
                        fundamental_factors.select(col).to_series().alias(col)
                    )
        
        # Add ZK market factors
        zk_factors = self.compute_market_factors_zk(raw_data)
        # Get only the computed factor columns (exclude original data columns)
        zk_cols = [col for col in zk_factors.columns 
                  if col not in ['Symbol', 'Dates'] and col in zk_factors.columns]
        if zk_cols:
            for col in zk_cols:
                if col in zk_factors.columns:
                    all_factors = all_factors.with_columns(
                        zk_factors.select(col).to_series().alias(col)
                    )
        
        # Add MLQ market factors
        market_factors = self.compute_market_factors_mlq(raw_data, past1m_data, past3m_data, past12m_data)
        # Get only the computed factor columns (exclude original data columns)
        market_cols = [col for col in market_factors.columns 
                      if col not in raw_data.columns and col in market_factors.columns]
        if market_cols:
            for col in market_cols:
                if col in market_factors.columns:
                    all_factors = all_factors.with_columns(
                        market_factors.select(col).to_series().alias(col)
                    )
    ...
    def compute_market_factors_zk(self, raw_data: pl.DataFrame) -> pl.DataFrame:
        ...
        Returns:
            Polars DataFrame with computed ZK market factors
        """
        # Start with the Symbol and Dates columns
        factors = raw_data.select(['Symbol', 'Dates'])
        
        # f_value = SE_TQ/MktCap_Comm_Pref (using totalEQ_0 and tmv as proxies)
        if (self.factor_dict['totalEQ_0'] in raw_data.columns and 
            self.factor_dict['tmv'] in raw_data.columns):
            factors = factors.with_columns(
                (pl.col(self.factor_dict['totalEQ_0']) / 
                 pl.col(self.factor_dict['tmv'])).alias('f_value')
            )
        
        # f_quality = GP_TQ/Assets_TQ (using earningsQ_0 and totalEQ_0 as proxies)
        if (self.factor_dict['earningsQ_0'] in raw_data.columns and 
            self.factor_dict['totalEQ_0'] in raw_data.columns):
            factors = factors.with_columns(
                (pl.col(self.factor_dict['earningsQ_0']).cast(pl.Float64) / 
        ...
        # f_div = DPS_Adj/AdjPrc (using div_0 and adjPrice)
        if (self.factor_dict['div_0'] in raw_data.columns and 
            self.factor_dict['adjPrice'] in raw_data.columns):
            factors = factors.with_columns(
                (pl.col(self.factor_dict['div_0']).cast(pl.Float64) / 
        ...
        # f_size = MktCap_Comm_Pref (using tmv)
        if self.factor_dict['tmv'] in raw_data.columns:
            factors = factors.with_columns(
                pl.col(self.factor_dict['tmv']).alias('f_size')
            )
        
        # f_vol = mean of BETA_1Y, VOL_1Y (using retStdev measures as proxies)
        if (self.factor_dict['retStdev_20_spot'] in raw_data.columns and 
            self.factor_dict['retStdev_120_spot'] in raw_data.columns):
            factors = factors.with_columns(
                ((pl.col(self.factor_dict['retStdev_20_spot']) + 
